...
public class BaselineGridTextView extends MaterialTextView {
    ...
    /**
     * When measured with an exact height, text can be vertically clipped mid-line.
     * Prevent this by
    ...
    private void computeLineHeight() {
        ...
        final float desiredLineHeight = (lineHeightHint > 0) ? lineHeightHint : lineHeightMultiplierHint * fontHeight;

        final int baselineAlignedLineHeight = (int) ((FOUR_DIP * (float) Math.ceil(desiredLineHeight / FOUR_DIP))
                + 0.5f);
